...
class InertiaStackInstaller
{
    ...
    public function run()
    {
        $this->output->writeln("<info>Setting up Laravel + Inertia.js + Vue.js stack for '{$this->projectName}'...</info>");
        ...
        try {
            ...
            $this->output->writeln("<info>Installing Inertia.js and Vue.js...</info>");
            $this->runShellCommand("cd {$this->projectPath()} && composer require inertiajs/inertia-laravel && npm install vue@next @inertiajs/inertia-vue3");

            $this->output->writeln("<info>Running migrations...</info>");
            $this->runShellCommand("cd {$this->projectPath()} && php artisan migrate");

            $this->output->writeln("<info>✓ Inertia.js + Vue.js stack setup completed!</info>");
            return true;
        } catch (\Exception $e) {
